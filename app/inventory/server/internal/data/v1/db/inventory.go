...
func (i *inventorys) UpdateStockSellDetailStatus(ctx context.Context, txn *gorm.DB, ordersn string, status int32) error {
	...
	if result.Error != nil {
		return errors.WithCode(baseCode.ErrDatabase, result.Error.Error())
	}
...
func (i *inventorys) CreateStockSellDetail(ctx context.Context, txn *gorm.DB, detail *do.StockSellDetailDO) error {
	...
	tx := db.Create(&detail)
	if tx.Error != nil {
		return errors.WithCode(baseCode.ErrDatabase, tx.Error.Error())
	}
	return nil
}
...
var _ data.InventoryStore = &inventorys{}
